...
本文位于 [[recommender-system]] 与 [[offline-rl]] 的交叉点,把 [[decision-transformer]] 与 [[rl-based-recsys|RL-based RecSys]] 引入 [[sequential-recommendation]]。它与 [[exposure-bias]] / [[debiasing]] 主题相关:不同于基于 [[inverse-propensity-score]] 或 [[distributionally-robust-optimization]] 的惩罚式 debiasing(如 DRO),CaseRec 改用 [[counterfactual-reasoning|counterfactual]] 数据增强 + [[user-simulation]] 来发掘未见兴趣。其 user simulator 思路属 [[model-based-rl]];Self-Improving 策略呼应 [[self-improvement]] 与 [[recommendation-simulator]] 的相关工作。基线层面与 [[gru4rec]]、[[sasrec]]、SQN、[[decision-transformer|DT]] 形成对照。
