...
社交平台的干预政策(如 [[recommender-systems|recommender-system]]、内容过滤、[[exposure-bias|曝光控制]])会无意中放大 [[echo-chamber|回音室]]、[[filter-bubble|过滤气泡]] 与 [[polarization|极化]],带来社会风险。现有评估主要依赖在线 A/B 测试,属于"反应式"(reactive):风险只能在部署后才暴露,反馈循环延迟且代价高昂,甚至可能造成不可逆的危害。已有的 [[llm-based-agents|LLM 社会模拟]] 提供了部署前替代方案,但当前方法存在三点不足:(1) 多数模拟未显式建模平台干预政策,难以刻画其效果;(2) 智能体设计严重依赖 [[prompt-engineering]] 而非对真实社交行为的真实建模,模拟可信度受限;(3) 缺乏利用模拟反馈来优化真实世界干预政策的原则化机制。
