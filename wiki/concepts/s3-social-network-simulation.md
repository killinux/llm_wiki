---
type: concept
subtype: method
tags: [social-simulation, llm-agent, multi-agent, agent-society, large-scale-simulation]
created: 2026-05-29
updated: 2026-05-29
sources: 1
---

# S3 Social Network Simulation

一种以 LLM 生成式社会 agent 为核心、结合真实社会环境与大规模仿真引擎的社会网络仿真方法,用于在可控环境中复现并研究群体层面的社会现象。

## 在本 wiki 中的出现

- [[2025-agentsociety-large-scale-social-simulation]]:一个整合 LLM 生成式社会 agent、真实城市-社会-经济环境与大规模分布式仿真引擎的社会模拟器,支持上万 agent 并复现极化、谣言、UBI、飓风、城市可持续性五类真实社会实验。

## 相关

- [[social-simulation]]
- [[llm-agent]]
- [[multi-agent-system]]
- [[large-scale-simulation]]
